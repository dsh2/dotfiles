...
if vim_plug_available
	...
	map <leader><leader>c :FzfCommands<cr>
	...
	nmap <leader>gc :tabnew<cr>:Git commit --verbose<cr>:only<cr>
	nmap <leader>gC :tabnew<cr>:Git commit --verbose --amend<cr>:only<cr>
	nmap <leader>gx dp:wincmd h<cr>:update<cr>:wincmd l<cr>
	nmap <leader>gs :tabnew<cr>:Git<cr>o
	...
	autocmd vimrc FileType gitcommit,fugitive map <buffer> ; dv
	...
	autocmd vimrc FileType nerdtree map <buffer> c o
	...
	autocmd vimrc FileType config,crontab,expect,exports,fstab,dosini,sshconfig,systemd,i3config,jq,resolv,gitconfig setlocal commentstring=#\ %s
...
autocmd vimrc BufNewFile,BufReadPost *.jq setfiletype config
" }}}
" Mappings {{{
...
" }}}
cabbr BB <buffer>
nmap <nowait> <leader>s :update<cr>\|:silent exec "!touch %"<cr>
...
autocmd vimrc BufReadPost */log/* normal G
autocmd vimrc BufReadPost *.log :set filetype=strace
"}}}
...
set splitright
nnoremap <c-w>l :vertical new<cr>
...
packadd cfilter
nmap <leader>< :vertical new<cr>cod
